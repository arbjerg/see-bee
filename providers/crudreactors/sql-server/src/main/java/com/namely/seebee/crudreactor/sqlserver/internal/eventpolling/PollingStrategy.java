...
public enum PollingStrategy {
    /**
     * All stream are polled in parallel with default transaction level isolation
     */
    FAST(false, false),

    /**
     * All stream are polled in parallel with snapshot transaction isolation level
     */
    ISOLATED(true, false),

    /**
     * All stream are polled in the same transaction which has default transaction level isolation
     */
    CONSISTENT(false, true),

    /**
     * All stream are polled in the same transaction which has snapshot transaction isolation level
     */
    SNAPSHOT(true, true)
